# bun-environment-runtime-adapters Specification

## Purpose
Bun owns the container and Kubernetes runtime capabilities — Docker/Podman
inspection, lifecycle, image/build/compose, logs/stats/events and cleanup, plus
Kubernetes cluster, image, secret, Helm and watch behaviour — behind the existing
`/api/*` route contracts. Observation failure is never treated as confirmed
absence, cleanup keeps its existing age/ownership/protection policy, and every
watch/poller has one cancellable server owner.
