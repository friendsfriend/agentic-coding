...
- **AND** active researcher runs are expired while their sessions remain live until workspace closure
...
- **AND** no duplicate workspace-close effect or lifecycle transition is created
...
- **AND** the researcher run remains active and its session remains live
...
- **THEN** the researcher run is expired only after the handoff is confirmed valid, while its session remains live until workspace closure
