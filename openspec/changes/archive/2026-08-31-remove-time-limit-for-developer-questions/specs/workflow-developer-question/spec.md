...
- **THEN** the interface SHALL return a bounded error or cancellation result no later than 24 hours after the question's creation, unless a shorter explicit CLI timeout is reached first
- **AND** the managed agent process SHALL not remain blocked indefinitely

#### Scenario: Default question expiry is 24 hours
- **WHEN** an authenticated managed agent creates a question without an explicit shorter CLI timeout
- **THEN** the durable question record SHALL have an expiry timestamp 24 hours after creation
- **AND** the question SHALL remain pending before that timestamp when no developer response has been received
