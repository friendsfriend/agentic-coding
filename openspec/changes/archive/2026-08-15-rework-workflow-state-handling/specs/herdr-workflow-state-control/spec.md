...
**Reason**: Raw phase overwrite preserves incompatible fields and can manufacture impossible lifecycle state.
**Migration**: Dashboard and CLI use validated `repair` with target step, expected revision, reason, confirmation, and explicit resume.

### Requirement: Run-bound recovery plan artifact
**Reason**: Separate recovery-agent plan adds another untrusted state-control path and duplicates deterministic repair validation.
**Migration**: Engine computes compatible repair targets directly; developer selects and confirms one through dashboard or CLI.
