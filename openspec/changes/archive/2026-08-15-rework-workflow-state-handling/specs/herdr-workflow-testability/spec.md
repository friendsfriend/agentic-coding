...
**Reason**: Direct `pane run` bypasses required Herdr managed-agent lifecycle and cannot provide consistent multi-runtime detection.
**Migration**: Every adapter uses `herdr agent start`, confirms with `agent get`, and sends prompt with `agent prompt`.

### Requirement: Prompt submission is verified and executed
**Reason**: Raw pane paste/Enter verification is runtime-specific and prohibited by managed lifecycle contract.
**Migration**: Herdr `agent prompt` is sole prompt transport after detected agent confirmation.
