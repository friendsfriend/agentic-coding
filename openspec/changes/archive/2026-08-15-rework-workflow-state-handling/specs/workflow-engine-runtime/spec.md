...
**Reason**: Legacy role-specific verbs let agents choose lifecycle transitions and duplicate workflow rules across skills and commands.
**Migration**: Managed agents receive generic `agentic-coding workflow handoff` in assignment; dashboard uses in-process actions; scripts migrate to new command surface.

### Requirement: Behavioral parity gate
**Reason**: Exact legacy phase/state behavior is intentionally replaced by validated step/run contracts.
**Migration**: Characterization changes to new command/view contracts, legacy migration fixtures, and end-to-end workflow-definition scenarios.
