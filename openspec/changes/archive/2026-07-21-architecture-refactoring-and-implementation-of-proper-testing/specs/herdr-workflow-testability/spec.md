...
Define the testable architecture of the `herdr-workflow` state machine: a
package decomposition that isolates pure decision logic from side effects behind
injectable seams, plus per-phase and per-workflow automated tests, while
preserving the CLI, `state.json`, and config contracts the dashboard depends on.
