...
Before launching a new agent process for a run, the engine SHALL attempt to resolve a live agent for the run's canonical identity — first from the run's persisted handle, then by the canonical agent name — and reuse that agent's pane when it is confirmed alive. A new tab or pane SHALL be created only when no live agent resolves or confirmation fails.
...
#### Scenario: Grouped roles get fresh identities per round
- **WHEN** a triage or verification round launches
- **THEN** each round's agents SHALL use their own round-scoped names and panes while rounds of the same step remain isolated from each other
