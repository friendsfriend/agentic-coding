...
- **WHEN** a workflow is started with the `fusion-propose` definition
- **THEN** the initial step SHALL be the fusion-planning step
...
- **WHEN** `plan-fusion` and `fusion-propose` are registered
...
- **WHEN** a plan-fusion or fusion-propose start declares fewer than 2 or more than 5 model routings, or duplicates a routing
...
The plan-fusion consolidation step SHALL be an agent step whose declared inputs are all validated planner drafts from the fan-out; it SHALL produce one consolidated OpenSpec proposal by creating the normal OpenSpec change artifacts, and its instructions SHALL direct it to reconcile conflicting approaches and record rejected alternatives rather than concatenate drafts. For `plan-fusion`, consolidation completion SHALL enter standard plan approval; for `fusion-propose`, it SHALL enter `core.closed` without approval.
...
- **WHEN** the consolidation agent in a `plan-fusion` workflow hands off `complete`
...
- **WHEN** the consolidation agent in a `fusion-propose` workflow hands off `complete`
...
The home dashboard SHALL expose the registered `plan-fusion` and `fusion-propose` workflows in the new workflow modal, SHALL provide the same task input available to standard and quick workflows, and SHALL pass the selected definition ID and entered task to workflow startup without changing the identifiers or behavior of existing workflow choices.
...
- **WHEN** a user opens the new workflow modal, chooses Fusion Proposal, and enters a task
- **THEN** the modal SHALL submit `workflowType` as `fusion-propose` and SHALL submit the entered task unchanged
...
- **WHEN** a user reaches the task step for a plan-fusion or fusion-propose workflow
...
When the dashboard starts `plan-fusion` or `fusion-propose` with a valid preset configuration, it SHALL derive ordered `planner-1` through `planner-N` routes for the configured N planner profiles and a `consolidator` route for `fusion.consolidate` before invoking the workflow engine.
