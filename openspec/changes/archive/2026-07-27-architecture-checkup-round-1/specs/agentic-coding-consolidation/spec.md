...
## Purpose
Living catalog of the `agentic-coding` workflow↔terminal-UI architecture. It records the agreed target — a single TypeScript umbrella binary that unifies the workflow engine and the dashboard views, with `otel-tui` kept separate — and a ranked backlog (R1–R8) of the structural findings behind that target. Later `architecture-checkup-round-N` changes implement backlog items against this catalog.
