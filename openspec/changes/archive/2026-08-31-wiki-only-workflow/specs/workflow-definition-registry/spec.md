...
The system SHALL define each workflow as an explicit, versioned graph of registered steps and legal outcomes rather than deriving behavior from phase names or array position. The catalog SHALL include explicit `standard-propose`, `fusion-propose`, and `wiki-only` graphs that reference the registered steps needed for their respective execution, while excluding implementation, verification, archive, delivery, and pull-request action/effect paths from proposal-only and wiki-only lifecycles.
...
- **THEN** standard, direct-apply, no-OpenSpec, plan-fusion, standard-propose, fusion-propose, and wiki-only definitions SHALL be registered through the public registry contract
