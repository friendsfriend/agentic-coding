...
The workflow dashboard detail view SHALL lay out its panel rows so that, at any terminal width, each row divides its usable width evenly between its columns after accounting for the inter-column gutter, every multi-column row's middle gutter SHALL occupy the same terminal column, and Git status SHALL be rendered inside the primary Change/overview panel rather than in a separate panel.
...
- **WHEN** the workflow dashboard detail view renders a row containing a single panel (`Current task`)
