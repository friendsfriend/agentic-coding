...
**Resolved during implementation:** `wikiVerificationPayload` stayed an engine helper (`runtime.ts`) — it reads real wiki filesystem state via `wikiRoot()`/`snapshotList`, which the step-module boundary (no filesystem handle, D4) does not admit; steps that need it call it through `onArrive`'s access to the engine-computed value. `profiles.ts`'s route check took the manifest-policy route: `enforceResearchReadOnlyRouting(routing, entryStepId)` now takes the target step id as a parameter instead of comparing to a literal `"core.research"`; both call sites (`cli.ts`, `tui/dash/engine.ts`) pass the resolved definition's `initial` step, which is `"research"`'s actual entry point — this reads as workflow-level (the manifest already exposes `initial`) without adding a fourth policy field for one call site.

## Task 6.1 audit: step-identity branches outside the engine

Per D5, each branch effect-runner.ts/assignment.ts/profiles.ts's design inventory named was audited against its actual required inputs *before* writing a hook, recorded here rather than widening `assignmentInputs` past `{ snapshot, run }`:

| Location (pre-refactor) | Branch | Inputs actually needed | Disposition |
| --- | --- | --- | --- |
| `effect-runner.ts:1054` `roundScoped` | `["core.triage","core.verification"].includes(stepId)` | `stepId` only | **Moved** — `StepBehavior.roundScoped` flag, read via `stepBehavior(stepId).roundScoped`. Also fixed the same duplicated literal in `cli.ts`'s `paneForRunFactory` (not separately inventoried, but the identical pattern). |
| `effect-runner.ts:1294` `assignmentFor` changed-files line | `run.stepId === "core.triage"` gating `changedFilesIn(snapshot)` | `snapshot` only (in scope), but the implementation lives in `runtime.ts` | **Left in place, recorded out of scope.** `changedFilesIn` is a non-trivial recursive git-status walker resident in `runtime.ts`; relocating it means either a `runtime.ts` file split (stage C's job, an explicit non-goal here) or duplicating the walker. Neither is worth it for one line of assignment text. |
| `effect-runner.ts:1323` `isResearchStep`/`isResearchWikiRole` (objective, interaction, permissions, checks, intro lines) | `snapshot.definition.id`, `run.stepId`, `run.role` | `{ snapshot, run }` | **Moved** — `core.research`'s and `core.wiki`'s `assignmentInputs` hooks. |
| `effect-runner.ts:1426,1442` `core.wiki` permissions/checks | `run.stepId === "core.wiki"` | `{ snapshot, run }` | **Moved** — `core.wiki`'s `assignmentInputs` hook. |
| `assignment.ts:184` (pre-refactor) `WIKI_ROLE_ASSET` map | `assignment.role` | `role` only | **Moved** — `StepBehavior.instructionAssetForRole`, declared on `core.wiki`. |
| `assignment.ts` (not separately inventoried) handoff guidance | `assignment.stepId === "core.research"` | `step.behavior` (already an argument to `renderAssignment`) | **Moved** — `StepBehavior.handoffNote`, declared on `core.research`. Found during the audit walk of `assignment.ts`, not in the original inventory's single line number. |
| `profiles.ts:502` (pre-refactor) `enforceResearchReadOnlyRouting` | `route.stepId === "core.research"` | The *target* step id, resolvable from the caller's already-resolved `definition.initial` | **Moved** — signature takes `entryStepId` as a parameter; callers pass `definition.initial` instead of a literal. |

Branches outside this file set that also compare against a `core.*`/`fusion.*` literal were audited and left in place; see `agentic-coding/docs/workflow-architecture.md`'s "Remaining step-identity matches outside `src/workflow/steps/`" table for the full list and reasons. That table enumerates, by function name and line range: ten `runtime.ts` functions outside the four this stage's design names (`start`, `migrateLegacy`, `recordResearchHandoff`, `developerAction`, `agentHandoff`, `effectResult`, `createRun`, `validateEffect`, `stopRoundAgents`, `validateFusionRouting` — `agentHandoff` is the largest concentration, covering the per-outcome completion handling this design's earlier drafts miscited as a function named `applyCommand`, which does not exist in this codebase), plus adapter launch context in `adapters.ts`, capability/authorization boundaries in `cli.ts`, a registration-time structural invariant in `registry.ts`/`cli.ts`, a schema-level terminal-status guard in `contracts.ts`'s `parseSnapshot()`, and all of `src/tui/` (stage D's job). It also names three `runtime.ts` functions (`validateTriageScope`, `validateSourceBaseline`, `wikiVerificationPayload`) that check `snapshot.definition.id`, not a step id, and so are out of this audit's scope entirely rather than "left in place" exceptions.
