          ...
          <box style={{ width: '100%', flexGrow: 1, minHeight: 0, flexDirection: 'row', gap: 1 }}> 
            ...
            <Panel title={`Change (${data().age} ago)`} accent={uiColors.primary} active={activePanel() === 0} style={{ width: '100%', flexGrow: 1, minHeight: 0 }}>  
              <ScrollableContent onScrollBoxReady={box => { changeScroll = box; }}>
                ...
                <Show when={data().state.workflowModules || data().state.phase !== 'closed'}><box flexDirection="row"><box width={7}><text fg={uiColors.textMuted}>TYPE</text></box><text fg={uiColors.textSecondary}>{(data().state.workflowModules ?? ['plan','plan-approval','apply-verify','developer-approval','archive'])[0] === 'plan' ? 'standard' : 'direct-apply'} · {(data().state.workflowModules ?? ['plan','plan-approval','apply-verify','developer-approval','archive']).length} modules</text></box></Show>
