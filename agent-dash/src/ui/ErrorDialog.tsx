...
export function ErrorDialog(props: { title: string; message: string; onClose: () => void; onScrollBoxReady?: (scrollBox: { scrollBy(dy: number): void }) => void }) {
  return <GenericModal title={`✗ ${props.title}`} titleColor={uiColors.error} help={[{ key: 'j/k', action: 'Scroll' }, { key: 'Esc', action: 'Close' }]} widthPercent={0.7} heightPercent={0.55} onBackdropClick={props.onClose}>
