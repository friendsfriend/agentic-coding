...
The dashboard's Trace tab renders `traces.jsonl` via the shared `otel-tui` package (`otel-tui/model/traceStore.ts` + `parser.ts`); see the root README for the standalone `otel-tui` binary. Herdr exports best-effort spans to `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, then `OTEL_EXPORTER_OTLP_ENDPOINT/v1/traces`. Prompt/model text, tool args/results, and repository content stay excluded; set `HERDR_TELEMETRY_MESSAGE_PREVIEW` only when bounded message previews are needed.
