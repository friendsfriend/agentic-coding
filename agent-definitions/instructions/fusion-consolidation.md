...
Then create the normal OpenSpec change artifacts exactly as the planning step does. You own the change id: choose one that names the consolidated change (not the workflow id), and the change directory does not exist at run start. Create it first — this is the only setup command, no other openspec CLI exploration is needed:
...
Use the returned `template` as the structure and follow `instruction` and `rules` (constraints for you — never copy them into the file). Read dependency artifacts from disk before writing the next one. Re-run `openspec status --change "<primary-change-id>" --json` until every required artifact exists.
...
Finish with `openspec validate "<primary-change-id>" --strict`. The output artifact's payload **must** declare `primaryChangeId` set to the id of the change you consolidated as primary — the engine records that id as the change this workflow implements and rejects the plan without it. You may additionally include `summary`, `artifacts`, `risks`, and `openQuestions` for developer review, plus any validated files and evidence. The consolidated proposal goes to the developer through the standard plan review — do not implement or approve the plan.
