...
export default function (pi: ExtensionAPI) {
  ...
  pi.on('agent_settled', (_event: any, ctx: any) => { traceEvent('settled'); end(operation); operation = undefined; write('pi_agent_settled'); if (oneShot) ctx.shutdown(); });
  ...
  pi.on('tool_execution_end', (event: any) => { const span = tools.get(event.toolCallId); if (span && captureContent()) span.attributes['herdr.content.tool_output'] = json(event.result); end(span, event.isError ? 'ERROR' : 'OK'); tools.delete(event.toolCallId); if (event.isError) { write('tool_error', { tool: event.toolName }); traceEvent('tool_error', { 'tool.name': event.toolName }, 'ERROR'); } });
