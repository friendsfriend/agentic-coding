...
import { stopServerStack, type ServerStack } from "../src/tui/index";

test("lifecycle steps transition through statuses", () => {
  beginStartup([{ id: "a", label: "A" }, { id: "b", label: "B" }]);
...
test("stopServerStack waits for the gRPC sidecar exit event", async () => {
  ...
  const sidecar = {
    kill: () => setTimeout(() => listeners.exit?.forEach(fn => fn()), 5),
    once: (event: string, fn: () => void) => { (listeners[event] ??= []).push(fn); },
  };
  let exitCode: number | undefined;
  await stopServerStack(
...
test("requestShutdown is idempotent and runs the registered stop sequence", async () => {
  finishStartup();
  let calls = 0;
  registerStopSequence(async () => { calls += 1; });
  requestShutdown();
  requestShutdown();
  expect(calls).toBe(1);
  expect(phase()).toBe("stopping");
