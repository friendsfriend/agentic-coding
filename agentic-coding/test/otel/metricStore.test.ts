...
function makeMetric(
	name: string,
	svc: string,
	type: "gauge" | "sum" | "histogram" = "gauge",
): MetricData {
	return {
		resource: { attributes: [] },
...
describe("MetricStore", () => {
	...
	it("filters by name query", () => {
		const store = new MetricStore([
			makeMetric("cpu.usage", "web"),
			makeMetric("mem.usage", "web"),
		]);
		store.setFilter("cpu");
	...
	it("sorts by type", () => {
		const store = new MetricStore([
			makeMetric("a", "srv", "histogram"),
			makeMetric("b", "srv", "gauge"),
		]);
		store.setSort("type");
	...
	it("aggregates data points", () => {
		const store = new MetricStore([makeMetric("cpu", "web")]);
		const stream = store.getStreams()[0];
		if (!stream) throw new Error("expected a stored stream");
		const agg = store.aggregate(stream);
