...
function fakeNoCredentialScript(dir: string): string {
  ...
  fs.writeFileSync(file, ['#!/bin/sh', 'echo "no-credential-required"', 'exit 0', ''].join('\n'), { mode: 0o700 });
  return file;
}

test('installAskpassShim creates a 0700 dir, 0700 shim and 0600 FIFOs, and cleans up', () => {
  const base = path.join(os.tmpdir(), `agentic-coding-askpass-test-${process.pid}`);
...
test('shim contract: relays the verbatim prompt and echoes the entered answer', async () => {
  ...
  try {
    ...
    await reader.close();
    expect(promptText.trim()).toBe("Enter passphrase for key '/home/test/.ssh/id_ed25519':");
...
test('shim times out instead of hanging when no answer is ever provided', async () => {
  ...
  const proc = Bun.spawn([shim.shimPath, "Enter passphrase for key '/home/test/.ssh/id_ed25519':"], {
    env: { ...process.env, AGENTIC_CODING_ASKPASS_DIR: shim.dir, AGENTIC_CODING_ASKPASS_TIMEOUT: '1' },
    stdout: 'pipe',
    stderr: 'pipe',
  });
  try {
...
test('runner detects the prompt, feeds the answer, and the command completes', async () => {
  ...
  try {
    ...
    const stdout = await runGitWithCredentials(dir, ['push', '--set-upstream', 'origin', 'feature/x'], {
      executable: fakeCredentialScript(dir),
      env: { AGENTIC_CODING_FAKE_PROMPT: "Enter passphrase for key '/home/test/.ssh/id_ed25519':" },
      prompt: async (prompt) => {
        prompts.push(prompt);
        return 'correct-passphrase';
      },
    });
    expect(prompts).toEqual(["Enter passphrase for key '/home/test/.ssh/id_ed25519':"]);
...
test('runner fails fast with an actionable diagnostic when no prompt provider is attached', async () => {
  ...
  try {
    ...
    await expect(promise).rejects.toThrow("Enter passphrase for key '/home/test/.ssh/id_ed25519'");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('masking heuristic masks passphrases and passwords but not usernames', () => {
  expect(maskingFor("Enter passphrase for key '/home/me/.ssh/id_ed25519':")).toBe(true);
...
test('credentialFailureMessage names the requested credential', () => {
  const message = credentialFailureMessage("Enter passphrase for key '/home/me/.ssh/id_ed25519':");
  expect(message).toContain("Enter passphrase for key '/home/me/.ssh/id_ed25519':");
