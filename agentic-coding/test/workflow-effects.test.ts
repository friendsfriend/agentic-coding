...
test('runner drains workspace and agent effects, then stops stale run after repair', async () => {
  ...
  try {
    ...
    const herdr = { call(command: string) { if (command === 'workspace') return { workspace: { workspace_id: 'workspace' } }; throw new Error(`unexpected ${command}`) } };
    ...
    engine.dispatch(repo, { type: 'operator.repair', workflowId: active.workflowId, revision: active.revision, targetStep: 'core.implementation', reason: 'test repair' }); await new EffectRunner(repo, engine, handlers).drain(); expect(adapter.stops).toBe(1); expect(engine.status(repo, 'effects').status).toBe('paused');
