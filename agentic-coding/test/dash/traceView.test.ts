...
test('parses workflow-engine traces.jsonl lines and groups them by change', () => {
  const line = (id: string, parentSpanId: string | null, status?: string) => JSON.stringify({
    traceId: 'a'.repeat(32), spanId: id.repeat(16), parentSpanId, name: `op-${id}`,
    startTimeUnixNano: '1000000', endTimeUnixNano: '2000000', status,
  ...
  const flatten = (nodes: ReturnType<TraceStore['getSpanTree']>): ReturnType<TraceStore['getSpanTree']> => nodes.flatMap(node => [node, ...flatten(node.children)]);
  const tree = new TraceStore(spans).getSpanTree('my-change');
  expect(tree).toHaveLength(1);
  expect(flatten(tree).some(node => node.span.name === 'op-2' && node.span.status.code === 2)).toBe(true);
});
