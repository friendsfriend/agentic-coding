...
import { CredentialsModal, credentialPromptBridge, pendingCredentialRequest } from "../../src/tui/dash/ui/CredentialsModal";
import { NewWorkflowModal } from "../../src/tui/dash/ui/NewWorkflowModal";

function key(name: string, extra: { meta?: boolean; shift?: boolean; sequence?: string } = {}): KeyEvent {
  return new KeyEvent({
    name,
    ctrl: false,
...
test("credentialPromptBridge resolves with the submitted answer and clears the pending request", async () => {
  ...
  const promise = bridge("Enter passphrase for key '/home/me/.ssh/id_ed25519':");
  const request = pendingCredentialRequest();
  expect(request).toBeTruthy();
  expect(request!.prompt).toBe("Enter passphrase for key '/home/me/.ssh/id_ed25519':");
