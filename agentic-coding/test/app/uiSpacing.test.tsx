...
import { advance, jumpTo, renderUntil } from "./support/terminal.ts";
...
test("every page keeps one blank row above and below its content", async () => {
	...
	try {
		...
		// page never pushes it in by more than the row's own card chrome — the
		// two-column block selector plus the single space after it.
		const menuRow = row(t, 3);
		expect(menuRow.length - menuRow.trimStart().length).toBeLessThanOrEqual(3);
