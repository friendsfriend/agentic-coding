...
describe('runHerdr', () => {
  test('parses the .result envelope', () => {
    const spy = mockSpawn(JSON.stringify({ result: { workspaces: [{ workspace_id: 'w1' }] } }));
    expect(runHerdr(['workspace', 'list'])).toEqual({ workspaces: [{ workspace_id: 'w1' }] });
    spy.mockRestore();
  });
  ...
  test('throws with stderr detail on non-zero exit', () => {
    ...
    expect(() => runHerdr(['pane', 'get', 'missing'])).toThrow(/pane not found/);
    spy.mockRestore();
  });
});
