...
export function setStepActive(id: string): void {
  ...
  setSteps(list => list.map(step => ({
    ...step,
    status: step.id === id ? 'active' : step.status === 'active' ? 'done' : step.status,
  })));
}

export function setStepDone(id: string): void {
  if (!inProgress()) return;
  setSteps(list => list.map(step => step.id === id ? { ...step, status: 'done' } : step));
}

export function setStepError(id: string, errMessage: string): void {
  if (!inProgress()) return;
  setSteps(list => list.map(step => step.id === id ? { ...step, status: 'error' } : step));
