...
export function calculateVisibleItems<T>(
	...
	const maxVisibleItems = Math.max(
		1,
		Math.floor(visibleHeight / estimatedItemHeight),
	);
	const pageStart =
		Math.floor(clampedSelected / maxVisibleItems) * maxVisibleItems;
	const startIdx = Math.max(
		0,
		Math.min(pageStart, Math.max(0, totalItems - maxVisibleItems)),
	);
