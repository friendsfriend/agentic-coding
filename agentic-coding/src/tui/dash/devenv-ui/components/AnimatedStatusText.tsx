...
const MODELS: Record<StatusAnimationIntent, StatusAnimationModel> = {
	...
	load: {
		label: "aurora + neutral · load",
		highlights: ["highlight1", "primary", "highlight3"],
	},
	ai: {
		label: "aurora + neutral · AI work",
		highlights: ["highlight1", "primary", "highlight2"],
	},
	script: { label: "amber + neutral · script", tone: "warning" },
};

export function statusAnimationModel(
	intent: StatusAnimationIntent,
): StatusAnimationModel {
	return MODELS[intent];
}

export function statusAnimationIntentForOperation(
	operation?: string,
...
export function statusAnimationIntentForText(
	text: string,
): StatusAnimationIntent {
...
export function AnimatedStatusText(props: AnimatedStatusTextProps) {
	const model = () =>
		statusAnimationModel(
			props.intent ?? statusAnimationIntentForText(props.text),
		);
	return (
		<InlineProgressAnimation
			text={props.text}
