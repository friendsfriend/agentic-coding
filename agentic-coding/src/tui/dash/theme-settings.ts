...
const configPath = () =>
	...
	resolve(process.env.HOME ?? "~", "dotfiles/pi/herdr-workflow.toml");
export const loadThemeName = () => {
	try {
		const match = readFileSync(configPath(), "utf8").match(
			/\[ui\][\s\S]*?^theme\s*=\s*"([^"]+)"/m,
		);
		return match?.[1] && themeNames.includes(match[1])
			? match[1]
			: "catppuccin";
	} catch {
		return "catppuccin";
...
export function saveThemeName(name: string) {
	...
	const next = /\[ui\]/.test(text)
		? /\[ui\][\s\S]*?^theme\s*=.*$/m.test(text)
			? text.replace(/(\[ui\][\s\S]*?^theme\s*=).*$/m, `$1 "${name}"`)
			: text.replace("[ui]", `[ui]\ntheme = "${name}"`)
		: `${text.trimEnd()}\n\n[ui]\ntheme = "${name}"\n`;
