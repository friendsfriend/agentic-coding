...
import catppuccin_frappe from "../../themes/catppuccin-frappe.json" with {
	type: "json",
};
import catppuccin_macchiato from "../../themes/catppuccin-macchiato.json" with {
	type: "json",
};
...
export type ThemeJson = {
	...
	theme: Record<
		string,
		string | number | { dark: string; light: string } | undefined
	>;
};
...
function allThemeJson(): Record<string, ThemeJson> {
	return systemThemeJson
		? { ...DEFAULT_THEME_JSON, ...customThemeJson, system: systemThemeJson }
		: { ...DEFAULT_THEME_JSON, ...customThemeJson };
}
...
function resolveValue(
	...
	if (
		...
		return resolveValue(
			theme,
			(value as { dark: unknown; light: unknown })[mode],
			fallback,
			mode,
			chain,
		);
	...
	return resolveValue(
		theme,
		theme.defs?.[value] ?? theme.theme[value],
		fallback,
		mode,
		[...chain, value],
	);
