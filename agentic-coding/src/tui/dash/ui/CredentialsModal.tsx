...
import { maskingFor, type CredentialPrompt } from '../../../workflow/credentials.ts';
...
const [pending, setPending] = createSignal<PendingCredentialRequest | undefined>(undefined);

export function pendingCredentialRequest(): PendingCredentialRequest | undefined {
  return pending();
}
...
export function CredentialsModal(props: { prompt: string; mask: boolean; value: string }) {
  return (
    <GenericModal
      ...
      heightPercent={0.35}
      zIndex={10}
      help={[{ key: 'Enter', action: 'Submit' }, { key: 'Esc', action: 'Cancel' }]}
    >
      <box width="100%" flexGrow={1} flexDirection="column" paddingTop={1} gap={1}>
        <text fg={uiColors.textMuted}>{props.mask ? 'Passphrase (masked)' : 'Answer'}</text>
        <box width="100%" height={1} backgroundColor={uiColors.bgBase} paddingLeft={1} flexDirection="row">
