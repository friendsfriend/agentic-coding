...
export type VerificationEntry = { role: string; status: string; durationSeconds?: number; model?: string; providerErrors: number; fallback: boolean };
  ...
  return <GenericModal title="Verification timeline" widthPercent={0.72} heightPercent={0.78} help={[{ key: 'j/k', action: 'Select' }, { key: 'Enter', action: 'View result' }, { key: 'Esc', action: 'Close' }]}>
    ...
    <SelectableList items={props.entries} selectedIndex={props.selected} renderItem={(entry, selected) => {
      ...
      return <box width="100%" height={2} flexDirection="column" paddingLeft={1} paddingRight={1}>
        ...
        <box height={1} flexDirection="row"><box flexGrow={1} overflow="hidden"><text fg={uiColors.textMuted}>{entry.model ?? 'default'}</text></box><text fg={uiColors.textMuted}>{entry.durationSeconds !== undefined ? `${entry.durationSeconds}s` : ''}</text></box>
      </box>;
    }} />
  </GenericModal>;
}
