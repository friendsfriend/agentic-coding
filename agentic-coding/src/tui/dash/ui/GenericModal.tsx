...
export function GenericModal(props: { title: string; titleColor?: string; fieldLabel?: string; children: JSX.Element; help: HelpEntry[]; summary?: SummaryEntry[]; summaryOnly?: boolean; widthPercent?: number; heightPercent?: number; heightLines?: number; step?: number; total?: number; search?: string; zIndex?: number; onBackdropClick?: () => void }) {
  ...
  return <Portal ref={(el) => { const portal = el as { position?: string; zIndex?: number }; portal.position = 'absolute'; portal.zIndex = props.zIndex ?? 0; }}><box position="absolute" top={0} left={0} width={dimensions().width} height={dimensions().height} flexDirection="column" justifyContent="center" alignItems="center" backgroundColor={RGBA.fromValues(0, 0, 0, 0.35)} onMouseUp={props.onBackdropClick}>
