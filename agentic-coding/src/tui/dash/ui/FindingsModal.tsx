...
export type FindingEvent = { type: string; severity?: string; path?: string; line?: number; detail?: string; evidence?: string; changedCode?: string; fix?: string };
export function FindingsModal(props: { title: string; events: FindingEvent[]; selected: number }) {
  ...
  const verdict = () => (props.events.some(event => event.severity === 'critical') ? 'FAIL' : 'PASS');
  const findings = () => props.events.filter(event => event.type === 'finding');
  return <GenericModal title={props.title} widthPercent={0.78} heightPercent={0.8} help={[{ key: 'j/k', action: 'Select' }, { key: 'Enter', action: 'Open editor' }, { key: 'Esc', action: 'Close' }]}>
    <box height={1} width="100%" flexDirection="row"><text fg={verdict() === 'PASS' ? uiColors.success : uiColors.error}>VERDICT (derived): {verdict()}</text><box flexGrow={1} /><text fg={uiColors.textMuted}>{findings().length} findings</text></box>
