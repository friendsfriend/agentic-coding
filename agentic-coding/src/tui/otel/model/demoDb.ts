...
function seedSpans(): SpanData[] {
	...
	const s2 = hex(16);
	spans.push(
		makeSpan(t1, s2, s1, "UsersService.findAll", "users-svc", 295, 85, false),
	);
	const s3 = hex(16);
	spans.push(
		makeSpan(t1, s3, s2, "Postgres.query", "users-svc", 290, 45, false),
	);
	const s4 = hex(16);
	...
	const s6 = hex(16);
	spans.push(
		makeSpan(t2, s6, s5, "OrdersService.create", "orders-svc", 195, 4800, true),
	);
	const s7 = hex(16);
	spans.push(
		makeSpan(
	...
	const s9 = hex(16);
	spans.push(
		makeSpan(t3, s9, s8, "LLM.generate", "herdr-agent", 145, 1800, false),
	);
	const s10 = hex(16);
	spans.push(
		makeSpan(t3, s10, s8, "tool.execute", "herdr-agent", 148, 500, false),
	);
...
function seedLogs(traceIds: string[]): LogData[] {
	...
	for (let i = 0; i < 50; i++) {
		const sevIdx =
			Math.random() < 0.7 ? 2 : Math.floor(Math.random() * sevs.length); // bias INFO
		const traceId =
			Math.random() < 0.5 && traceIds.length
				? traceIds[Math.floor(Math.random() * traceIds.length)]
				: undefined;
		logs.push({
			resource: { attributes: [] },
			scope: { name: "demo", version: "1.0" },
			timeUnixNano: (
				BigInt(Date.now() - Math.floor(Math.random() * 600) * 1000) * 1_000_000n
			).toString(),
