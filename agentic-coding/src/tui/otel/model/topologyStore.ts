	...
	private buildGraph(): ServiceNode[] {
		...
		for (const span of this.spans) {
			...
			if (span.status.code === 2)
				errorCountMap.set(svc, (errorCountMap.get(svc) ?? 0) + 1);
			const dur = Math.max(
				0,
				Number(
		...
		for (const span of this.spans) {
			...
			if (parent && child) {
				if (!parent.childIds.includes(span.serviceName))
					parent.childIds.push(span.serviceName);
				if (!child.parentIds.includes(parentSvc))
					child.parentIds.push(parentSvc);
			}
		}
		...
		for (const [svc, count] of spanCountMap) {
			...
			const durs = durationMap.get(svc) ?? [];
			node.avgDurationMs = durs.length
