...
const runtimeColor = (share: number) => {
	...
	const position = Math.max(0, Math.min(1, share)) * (stops.length - 1);
	const index = Math.floor(position);
	const amount = position - index;
	const from = stops[index];
	const to = stops[Math.min(index + 1, stops.length - 1)];
	const rgb = from.map((value, channel) =>
		Math.round(value + (to[channel] - value) * amount),
	);
...
export function TraceTreeView(props: {
	...
	const data = createMemo(() => {
		...
		const visit = (
			...
			for (const [index, node] of nodes.entries()) {
				...
				parentShares.set(
					spanKey(node),
					parent ? durationMs(node) / Math.max(1, durationMs(parent)) : 1,
				);
		...
		timeline.sort((a, b) =>
			Number(
				BigInt(a.node.span.startTimeUnixNano) -
					BigInt(b.node.span.startTimeUnixNano),
			),
		);
		const actual = timeline.filter(
			(item) => item.node.span.scope.name !== "viewer",
		);
		if (!actual.length)
			return {
				items: [],
				offsets: new Map<
					string,
					{ offset: number; share: number; parentShare: number; color: string }
				>(),
			};
		const start = BigInt(actual[0].node.span.startTimeUnixNano);
		const end = actual.reduce((latest, item) => {
			const value = BigInt(item.node.span.endTimeUnixNano);
		...
		for (const item of timeline) {
			...
			offsets.set(spanKey(item.node), {
				offset:
					Number(
						(BigInt(item.node.span.startTimeUnixNano) - start) / 1_000_000n,
					) / total,
				share,
				parentShare,
	...
	return (
		<box flexDirection="column" width="100%" height="100%">
						...
						{(item, index) => {
							...
							const status =
								node.span.status.code === 2
									? "negative"
									: node.span.status.code === 1
										? "warning"
										: "positive";
							const runtime = data().offsets.get(spanKey(node)) ?? {
								offset: 0,
								share: 0,
								parentShare: 0,
							...
							const offset = Math.max(
								0,
								Math.min(barWidth(), Math.round(runtime.offset * barWidth())),
							);
							const width = runtime.share
								? Math.max(1, Math.round(runtime.share * barWidth()))
								: 0;
									...
									<box height={1} flexDirection="row">
										...
										>
											<box
												...
												flexShrink={0}
												onMouseUp={() =>
													node.children.length > 0 && props.onToggle(node, path)
												}
											>
												<HighlightedText
													text={childMark}
													highlight={
														node.children.length ? "secondary" : "primary"
													}
												/>
											</box>
											<HighlightedText
										...
										>
											...
											>{`${(runtime.parentShare * 100).toFixed(1).padStart(5)}% `}</text>
											<text fg={runtime.color}>{bar}</text>
										</box>
										<box width={1} flexShrink={0} />
			...
			{data().items.length === 0 && (
				<box paddingLeft={1}>
					<text fg={uiColors.textMuted}>Select a trace to view tree</text>
				</box>
			)}
		</box>
	);
