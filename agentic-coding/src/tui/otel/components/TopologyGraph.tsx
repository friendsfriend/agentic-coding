...
export function TopologyGraph(props: {
	...
	const graphHeight = () =>
		Math.max(
			6,
			Math.max(...layers().map(([, nodes]) => nodes.length), 1) *
				(cardHeight + 1) +
				1,
		);
										...
										return (
											...
											>
												<box
													...
													position="absolute"
													backgroundColor={
														selected() ? uiColors.accent : uiColors.border
													}
												/>
												...
												>
													<text fg={uiColors.textMuted}>
														{ln.node.spanCount} spans ·{" "}
														{ln.node.avgDurationMs.toFixed(1)}ms
													</text>
												</box>
												<box paddingLeft={2}>
													<text
