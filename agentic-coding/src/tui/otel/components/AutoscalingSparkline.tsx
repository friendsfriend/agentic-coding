...
function rows(values: number[], width: number, height: number): string[] {
	...
	const points = [
		...Array(Math.max(0, width - display.length)).fill(NaN),
		...display,
	];
	...
	for (let x = 0; x < width * 2; x++) {
		const pos = x / 2,
			left = Math.floor(pos),
			right = Math.min(points.length - 1, left + 1);
		const a = points[left],
			b = points[right];
		...
		const y =
			...
			1 -
			Math.round(
				clamp((value - min) / (max - min || 1), 0, 1) * (height * 4 - 1),
			);
		const dot = dots[y % 4]?.[x % 2] ?? 0;
		masks[Math.floor(y / 4)][Math.floor(x / 2)] |= dot;
	}
	return masks.map((row) =>
		row
			.map((mask) => (mask ? String.fromCharCode(0x2800 + mask) : " "))
			.join(""),
	);
}

export function AutoscalingSparkline(props: {
	values: number[];
	color?: string;
	...
	const renderAfter = function (this: BoxRenderable, buffer: OptimizedBuffer) {
		const width = Math.max(1, this.width ?? 0),
			height = Math.max(1, this.height ?? 0);
		const fg = RGBA.fromHex(props.color ?? uiColors.primary),
			bg = RGBA.fromHex(props.backgroundColor ?? uiColors.bgMantle);
		buffer.fillRect(this.screenX, this.screenY, width, height, bg);
		rows(props.values, width, height).forEach((line, y) => {
			buffer.drawText(line, this.screenX, this.screenY + y, fg, bg);
		});
	};
	return (
		<box
			...
			backgroundColor={props.backgroundColor ?? uiColors.bgMantle}
			style={{ flexGrow: 1, minWidth: 1, height: "100%", overflow: "hidden" }}
		/>
	);
}
