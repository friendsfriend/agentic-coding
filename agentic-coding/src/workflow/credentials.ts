...
function makeFifo(file: string): void {
	const result = Bun.spawnSync(["mkfifo", "-m", "600", file], {
		stderr: "ignore",
	});
	if (result.exitCode !== 0) {
		try {
			if (fs.statSync(file).isFIFO()) return;
		} catch {
...
export function installAskpassShim(
	dir = path.join(os.tmpdir(), "agentic-coding-askpass"),
): AskpassShim {
...
export async function runGitWithCredentials(
	...
	const proc = Bun.spawn([options.executable ?? "git", "-C", cwd, ...args], {
		env,
		stdout: "pipe",
		stderr: "pipe",
	});
...
async function waitForRequest(
	proc: Subprocess,
	shim: AskpassShim,
	holder: { reader?: Subprocess },
): Promise<string | undefined> {
	const reader = Bun.spawn(["cat", shim.requestFifo], {
		stdout: "pipe",
		stderr: "ignore",
	});
	...
	if (outcome === undefined && reader.exitCode === null) {
		try {
			reader.kill();
		} catch {
			/* already gone */
