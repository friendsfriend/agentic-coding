...
export function renderAssignment(step: Readonly<StepDefinition>, assignment: Assignment, assetRoot = path.join(AGENT_DEF_DIR, 'instructions')): RenderedAssignment {
  ...
  const dynamic = [
    ...
    '', '## Output contract', ...(assignment.output ? [`Path: ${assignment.output.path}`, `Schema: ${assignment.output.schemaId}@${assignment.output.schemaVersion}`, `Maximum bytes: ${assignment.output.maxBytes}`, 'Write exactly this envelope shape:', '```json', envelope!, '```', ...(assignment.output.schemaId === 'core.triage-plan' ? ['Payload: `roles` array; each item requires string `role`, non-empty string `reason`, and non-empty repository-relative string array `files`; optional `hunks` maps files to hunk IDs 1..8.'] : assignment.output.schemaId === 'core.findings' ? ['Payload: `findings` array; each item requires unique string `id`, `severity` (`critical`, `warning`, or `info`), non-empty string `detail`, repository-relative `path`, and integer `line` (1-based line in the current file).'] : ['Payload: JSON value required by step instructions.'])] : ['No output artifact.']),
    '', `Allowed outcomes: ${assignment.allowedOutcomes.join(', ')}`, '', '## Handoff', 'Finish the run by reporting exactly one outcome with the workflow CLI:', '```bash', handoff, 'agentic-coding workflow handoff --outcome blocked --message "reason"', 'agentic-coding workflow handoff --outcome failed --message "diagnostic"', '```',
  ].join('\n');
