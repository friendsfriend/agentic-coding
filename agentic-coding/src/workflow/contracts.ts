...
export type WorkflowStatus = 'active' | 'paused' | 'attention-required' | 'completed' | 'closed';
export type RunStatus = 'pending' | 'working' | 'completed' | 'blocked' | 'failed' | 'expired';
export type EffectStatus = 'pending' | 'running' | 'retry' | 'completed' | 'failed' | 'expired';
export type RuntimeId = 'pi' | 'opencode' | 'opencode-v2' | (string & {});
export type AdapterCapability = 'interactive' | 'prompt' | 'persistent-session' | 'run-environment' | 'observe' | 'read-only' | 'shell' | 'edit' | 'runtime-bridge';
export type EffectKind = 'workspace.setup' | 'artifact.write' | 'agent.launch' | 'agent.prompt' | 'agent.stop' | 'notification.show' | 'openspec.validate' | 'delivery.commit' | 'delivery.push' | 'pull-request.create' | 'workspace.close' | 'workspace.cleanup';
export type JsonPrimitive = string | number | boolean | null;
export type JsonValue = JsonPrimitive | JsonValue[] | { [key: string]: JsonValue };

export interface ContractError { path: string; message: string }
export class ContractFailure extends Error {
  constructor(readonly contractId: string, readonly issues: ContractError[]) {
    super(`${contractId}: ${issues.slice(0, 8).map(issue => `${issue.path}: ${issue.message}`).join('; ')}`);
  }
}
export interface Contract<T> { readonly id: string; readonly version: number; parse(value: unknown): T }

function object(value: unknown, at: string): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new ContractFailure(at, [{ path: at, message: 'expected object' }]);
...
function strings(value: unknown, at: string): string[] {
  if (!Array.isArray(value) || value.some(item => typeof item !== 'string')) throw new ContractFailure(at, [{ path: at, message: 'expected string array' }]);
  return value as string[];
}
function enumValue<const T extends readonly string[]>(value: unknown, at: string, values: T): T[number] {
...
export interface ResolvedProfile {
  name: string; runtime: RuntimeId; executable: string; model?: string; agent?: string; thinking?: string;
  tools: readonly string[]; extensions: readonly string[]; readOnly: boolean; capabilities: readonly AdapterCapability[]; digest: string;
}
export interface WorkflowRouting { defaultProfile: string; routes: readonly { stepId: string; role?: string; profile: ResolvedProfile }[]; /** @deprecated ignored legacy snapshot data */ diversity?: readonly { routes: string[]; satisfied: boolean }[] }
export interface StepAttemptState {
  attempt: number; mode?: 'apply' | 'fix' | 'review-fix'; activeRunIds: string[]; completedRunIds: string[];
  selectedRoles: string[]; testRunStarted: boolean; context?: JsonValue; results: Array<{ runId: string; role: string; critical: number; outputDigest?: string }>;
}
export interface WorkflowMetadata {
  repository: string; worktree: string; changeId: string; branch: string; baseBranch: string; baseCommit: string;
  workspace?: string; task?: string; ticket?: string; createdAt: string; updatedAt: string; stepEnteredAt: string;
}
...
export interface WorkflowRun {
  ...
  profile: ResolvedProfile; issuedRevision: number; allowedOutcomes: readonly ('complete' | 'blocked' | 'failed')[]; capabilityHash: string; capabilityExpiresAt: string; assignmentPath: string; outputPath?: string;
  outputSchema?: { id: string; version: number }; outputDigest?: string; handle?: AgentHandle; createdAt: string; completedAt?: string;
}
export interface AgentHandle { runtime: RuntimeId; name: string; paneId: string; tabId?: string; sessionId?: string }
export interface WorkflowEffect {
  id: string; workflowId: string; revision: number; kind: EffectKind; idempotencyKey: string; payload: JsonValue; status: EffectStatus;
  attempts: number; maxAttempts: number; lease?: string; leaseExpiresAt?: string; nextAttemptAt?: string; lastError?: string;
}
...
export interface WorkflowView {
  workflowId: string; changeId: string; revision: number; definition: DefinitionPin & { label: string }; status: WorkflowStatus;
  repository: string; worktree: string; branch: string; baseCommit: string; workspace?: string; createdAt: string; updatedAt: string;
  currentStep: { id: string; label: string; attempt: number; enteredAt: string }; runs: Array<{ id: string; stepId: string; role: string; attempt: number; status: RunStatus; runtime: RuntimeId; profile: string; model?: string; paneId?: string; outputPath?: string; outputDigest?: string }>;
  routing: WorkflowRouting; effects: Array<{ id: string; kind: EffectKind; status: EffectStatus; attempts: number; lastError?: string }>;
  observations: Array<{ runId: string; runtime: RuntimeId; status: string; at: string }>;
  health: { valid: boolean; attention: string[]; diagnostic?: string }; availableActions: WorkflowActionView[];
}
...
export const commandContract: Contract<WorkflowCommand> = {
  id: 'core.workflow-command', version: 1,
...
function profile(value: unknown, at: string): ResolvedProfile { const input = object(value, at); const runtime = text(input.runtime, `${at}.runtime`, 64); const capabilities = strings(input.capabilities, `${at}.capabilities`) as AdapterCapability[]; const allowed: AdapterCapability[] = ['interactive', 'prompt', 'persistent-session', 'run-environment', 'observe', 'read-only', 'shell', 'edit', 'runtime-bridge']; if (capabilities.some(item => !allowed.includes(item))) throw new ContractFailure('core.agent-profile', [{ path: `${at}.capabilities`, message: 'unknown adapter capability' }]); if (typeof input.readOnly !== 'boolean') throw new ContractFailure('core.agent-profile', [{ path: `${at}.readOnly`, message: 'expected boolean' }]); return { name: text(input.name, `${at}.name`), runtime, executable: text(input.executable, `${at}.executable`), ...(input.model === undefined ? {} : { model: text(input.model, `${at}.model`) }), ...(input.agent === undefined ? {} : { agent: text(input.agent, `${at}.agent`) }), ...(input.thinking === undefined ? {} : { thinking: text(input.thinking, `${at}.thinking`) }), tools: strings(input.tools, `${at}.tools`), extensions: strings(input.extensions, `${at}.extensions`), readOnly: input.readOnly, capabilities, digest: text(input.digest, `${at}.digest`) } }
...
export function parseSnapshot(value: unknown): WorkflowSnapshot {
  ...
  const snapshot: WorkflowSnapshot = {
    schemaVersion: integer(input.schemaVersion, '$.schemaVersion', 1) as 1, workflowId: text(input.workflowId, '$.workflowId'), revision: integer(input.revision, '$.revision'),
    definition: { id: text(definition.id, '$.definition.id'), version: integer(definition.version, '$.definition.version', 1), digest: text(definition.digest, '$.definition.digest') },
    status: enumValue(input.status, '$.status', ['active', 'paused', 'attention-required', 'completed', 'closed']), currentStep: text(input.currentStep, '$.currentStep'),
    step: { attempt: integer(step.attempt, '$.step.attempt', 1), ...(step.mode === undefined ? {} : { mode: enumValue(step.mode, '$.step.mode', ['apply', 'fix', 'review-fix']) }), activeRunIds: strings(step.activeRunIds, '$.step.activeRunIds'), completedRunIds: strings(step.completedRunIds, '$.step.completedRunIds'), selectedRoles: strings(step.selectedRoles, '$.step.selectedRoles'), testRunStarted: (() => { if (typeof step.testRunStarted !== 'boolean') throw new ContractFailure('core.workflow-snapshot', [{ path: '$.step.testRunStarted', message: 'expected boolean' }]); return step.testRunStarted })(), ...(step.context === undefined ? {} : { context: JSON.parse(JSON.stringify(step.context)) as JsonValue }), results: (() => { if (!Array.isArray(step.results)) throw new ContractFailure('core.workflow-snapshot', [{ path: '$.step.results', message: 'expected array' }]); return step.results.map((entry, i) => { const item = object(entry, `$.step.results[${i}]`); return { runId: text(item.runId, `$.step.results[${i}].runId`), role: text(item.role, `$.step.results[${i}].role`), critical: integer(item.critical, `$.step.results[${i}].critical`), ...(item.outputDigest === undefined ? {} : { outputDigest: text(item.outputDigest, `$.step.results[${i}].outputDigest`) }) }; }) })() },
  ...
  if (new Set(snapshot.step.activeRunIds).size !== snapshot.step.activeRunIds.length) throw new ContractFailure('core.workflow-snapshot', [{ path: '$.step.activeRunIds', message: 'duplicate run ID' }]);
  return snapshot;
}
