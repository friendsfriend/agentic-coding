import type { MetricData, MetricDataPoint } from "../../contracts/telemetry.ts";
 ...
 */
export function normalizeStatsDMetrics(
	lines: string[],
	source: string,
): MetricData[] {
	...
	for (const line of lines) {
		...
		const key = `${parsed.name}:${Object.entries(parsed.tags)
			.sort()
			.map(([k, v]) => `${k}=${v}`)
			.join(",")}`;
		const existing = aggregator.get(key);

		const tagAttrs = Object.entries(parsed.tags).map(([k, v]) => ({
			key: k,
			value: v,
		}));
		...
		if (existing) {
			...
			const otelType =
				parsed.type === "g" || parsed.type === "c"
					? "gauge"
					: parsed.type === "ms" || parsed.type === "h"
						? "sum"
						: "gauge";
			aggregator.set(key, {
