import type { ActionRun } from "./action-run.ts";
...
export function actionRunDisplayLabel(
	...
	if (title) {
		...
		return label
			? `${label} ${run.targetLabel || formatProfileLabel(run.profile)}`
			: title;
	}
	return [actionTypeLabel(run.action), run.targetLabel ?? run.appIdent]
		.filter(Boolean)
		.join(" ");
}
