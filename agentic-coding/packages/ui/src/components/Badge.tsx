...
export interface BadgeProps {
	...
	/** Shorthand for `animatedHighlights`: `"aurora"` animates with the default
	 * palette, `"static"` never animates. */
	animation?: "static" | "aurora" | (string & {});
...
const rememberSnapshot = (key: string, snapshot: BadgeSnapshot) => {
	...
	if (transitionSnapshots.size > 1000) {
		const oldest = transitionSnapshots.keys().next().value;
		if (oldest !== undefined) transitionSnapshots.delete(oldest);
	}
};
const sameSnapshot = (left: BadgeSnapshot, right: BadgeSnapshot) =>
...
export function Badge(props: BadgeProps) {
	...
	const snapshot = (): BadgeSnapshot => ({
		...
		color: parseColor(
			props.color ?? highlightColor(props.highlight ?? "highlight"),
		),
	...
	const cached = props.transitionKey
		? transitionSnapshots.get(props.transitionKey)
		: undefined;
	...
	const wipeTimeline = useTimeline({
		autoplay: false,
		...
		on(
			...
			() => {
				...
				if (!props.transitionKey) {
					// Without a stable identity there is nothing to wipe between: swap
					// immediately, so a host whose timeline is not running (tests, a
					// paused animation loop) never keeps the previous text on screen.
					setPrevious(next);
					setCurrent(next);
					wipe.progress = 1;
					root?.requestRender();
				...
				setPrevious(current());
				setCurrent(next);
				rememberSnapshot(props.transitionKey, next);
				startWipe();
			},
			{ defer: true },
		),
	...
	const auroraTimeline = useTimeline({
		autoplay: false,
		duration: 1600,
		loop: true,
	});
	...
	const animatedHighlights = () =>
		...
		(props.animation === "aurora" ? DEFAULT_ANIMATION_HIGHLIGHTS : undefined);
	const palette = createMemo(() =>
		props.animatedTone
			? createTonePalette(props.animatedTone)
			: animatedHighlights()
				? createAuroraPalette(animatedHighlights() as AnimationHighlights)
				: undefined,
	);
	createEffect(() => {
		if (props.animatedTone || animatedHighlights()) {
			auroraTimeline.play();
		} else {
	...
	const snapshotWidth = (badge: BadgeSnapshot) =>
		badge.text.length + (badge.appearance === "badge" ? 4 : 0);
	const width = () =>
		Math.max(snapshotWidth(previous()), snapshotWidth(current()));

	const renderAfter = function renderAfter(
		this: BoxRenderable,
		...
		for (let x = 0; x < badgeWidth; x++) {
			...
			const textIndex = x - 2;
			const character =
				textIndex >= 0 && textIndex < badge.text.length
					? badge.text[textIndex]
					: " ";
	...
	return (
		<box
			...
			style={{ width: width(), height: 1, flexShrink: 0 }}
		/>
	);
}
