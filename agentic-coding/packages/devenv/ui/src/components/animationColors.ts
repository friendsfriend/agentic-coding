...
export function createAuroraPalette(
	highlights: AnimationHighlights,
	steps = 8,
) {
	const colors = highlights.map((highlight) =>
		RGBA.fromHex(highlightColor(highlight)),
	) as [RGBA, RGBA, RGBA];
	return [
		...Array.from({ length: steps }, (_, index) =>
			mix(colors[0], colors[1], index / steps),
		),
		...Array.from({ length: steps }, (_, index) =>
			mix(colors[1], colors[2], index / steps),
		),
		...Array.from({ length: steps }, (_, index) =>
			mix(colors[2], colors[0], index / steps),
		),
	];
}
...
export function createTonePalette(highlight: Highlight, steps = 8) {
	...
	return [
		...Array.from({ length: steps }, (_, index) =>
			mix(dark, base, index / steps),
		),
		...Array.from({ length: steps }, (_, index) =>
			mix(base, neutral, index / steps),
		),
		...Array.from({ length: steps }, (_, index) =>
			mix(neutral, dark, index / steps),
		),
	];
}
