...
const [activeThemeName, setActiveThemeNameSignal] = createSignal('tokyonight');
...
function activeTheme(): ThemeJson {
  return themeMap[activeThemeName()] ?? themeMap.tokyonight!;
}
...
export function themeColorForTheme(name: string, key: string, fallback: string): string {
  ...
  return theme ? resolveColor(theme, key, fallback) : fallback;
}
