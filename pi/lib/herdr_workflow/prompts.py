...
def role_prompt(role, change, verification_round=None, workflow_type=None, task=None):
    ...
    if role == "security-verifier":
        ...
        return f"Silent security verifier for {change} round {verification_round}. Read {context}. Review changed trust boundaries for introduced injection, auth, secret, crypto, and input-validation defects. Write JSONL findings plus final PASS/FAIL verdict to {report}, then run herdr-workflow verification-result --repo . --change {change} --role security-verifier. No chat output. Only report actual defects and issues. Do not include findings that merely confirm code was implemented correctly — if nothing is wrong, write only the PASS verdict with no findings." + persistent
    ...
    if role == "quality-verifier":
        ...
        return f"Silent code quality verifier for {change} round {verification_round}. Read {context}. Run focused formatting, lint, and type checks; review changed code for concrete correctness and maintainability defects. Write JSONL findings plus final PASS/FAIL verdict to {report}, then run herdr-workflow verification-result --repo . --change {change} --role quality-verifier. No chat output. Only report actual defects and issues. Do not include findings that merely confirm code was implemented correctly — if nothing is wrong, write only the PASS verdict with no findings." + persistent
    if role == "performance-verifier":
        ...
        return f"Silent performance verifier for {change} round {verification_round}. Read {context}. Review changed hot paths for measurable query, I/O, CPU, blocking, and memory regressions. Write JSONL findings plus final PASS/FAIL verdict to {report}, then run herdr-workflow verification-result --repo . --change {change} --role performance-verifier. No chat output. Only report actual defects and issues. Do not include findings that merely confirm code was implemented correctly — if nothing is wrong, write only the PASS verdict with no findings." + persistent
    ...
    if role == "usability-verifier":
        ...
        return f"Silent usability verifier for {change} round {verification_round}. Read {context}. Review changed frontend and asset files for introduced visual consistency, accessibility, responsive layout, design-system, component-state, and hardcoded-style defects. Write JSONL findings plus final PASS/FAIL verdict to {report}, then run herdr-workflow verification-result --repo . --change {change} --role usability-verifier. No chat output. Only report actual defects and issues. Do not include findings that merely confirm code was implemented correctly — if nothing is wrong, write only the PASS verdict with no findings." + persistent
    if role == "test-verifier":
        ...
        return f"Silent full-suite test verifier for {change} round {verification_round}. Read {context}. Find and run repository's complete configured test suite without filters; PASS requires successful suite and regression coverage. Write JSONL findings plus final PASS/FAIL verdict to {report}, then run herdr-workflow verification-result --repo . --change {change} --role test-verifier. No chat output. Only report actual defects and issues. Do not include findings that merely confirm code was implemented correctly — if nothing is wrong, write only the PASS verdict with no findings." + persistent
    if role == "archive":
